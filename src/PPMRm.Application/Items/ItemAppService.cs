...
namespace PPMRm.Items
{
    public class ItemAppService : ApplicationService, IItemAppService
    {
        ...
        {
            using var session = DocumentStore.LightweightSession();
            var totalCount = await session.Query<Item>().CountAsync();
            var items = await session.Query<Item>().OrderBy(i => i.Name).Skip(input.SkipCount).Take(input.MaxResultCount).ToListAsync();
